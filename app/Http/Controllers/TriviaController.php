...
{
    abort_if($content->type !== 'trivia', 404);
    ...
    abort_if(!$correctOption, 500, 'Trivia has no correct answer configured');

    $normalize = fn($text) => strtolower(trim(preg_replace('/[^\w\s]/', '', $text)));

    $userAnswer = $normalize($request->answer);
    $correctAnswer = $normalize($correctOption->option_text);
